...
namespace NodeCanvas.Tasks.Actions
{
    ...
    public class Wander : ActionTask<NavMeshAgent>
    {
        ...
        protected override void OnUpdate() {
            if (agent.isActiveAndEnabled && agent.isOnNavMesh && agent.hasPath && !agent.pathPending && agent.remainingDistance <= agent.stoppingDistance + keepDistance.value ) {
                if ( repeat ) {
                    DoWander();
                } else {
                    EndAction();
        ...
        void DoWander() {
            ...
            NavMeshHit hit;
            if (agent.isActiveAndEnabled && agent.isOnNavMesh && NavMesh.SamplePosition(wanderPos, out hit, agent.height * 2, NavMesh.AllAreas) ) {
